...
public class AuthController : ControllerBase
{
    ...
    public IActionResult ValidarToken([FromBody] ValidarTokenRequest request)
    {
        if (request == null || string.IsNullOrWhiteSpace(request.Token))
            return Unauthorized(new { valido = false, message = "Token não enviado" });
        ...
        try
        {
            ...
            var minutosRestantes = (exp - DateTime.UtcNow).TotalMinutes;

            // claims do token
            var usuarioId = jwtToken.Claims.FirstOrDefault(x => x.Type == "id")?.Value;
            var usuarioNome = jwtToken.Claims.FirstOrDefault(x => x.Type == "usuario")?.Value;
